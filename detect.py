...
class SceneDataset:
    def __init__(self, image_path, crop_size=(None, None)):
        ...
        self.names = [
            p for p in os.listdir(image_path) if p.endswith(args.image_extension)
        ]
...
def extract(dataset, save_path):
    ...
    if args.mode == "nms":
        extract = partial(
            model.features, kind="nms", window_size=args.window, cutoff=0.0, n=args.n
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="nms",
        help=(
            "Whether to extract features using the non-maxima suppresion mode or "
    ...
    parser.add_argument(
        "h5_path",
        help=(
            "Directory where keypoints.h5 and descriptors.h5 will be stored. This"
